...
@ApiTags("medical-reference-linked-list")
@UseGuards(AuthenticatedHeaderGuard, PoliciesGuard)
@ApiSecurity("ApiKeyAuth", ["token"])
@Controller("medical-reference")
export class MedicalReferenceLinkedListController {
  constructor(private readonly service: MedicalReferenceService) {}
  ...
  @ApiQueries(["locale"])
  @ApiCreatedResponse({ type: MedicalReferenceDto })
  ...
  @ApiParam({
    name: "from",
    type: "string",
    description: "_id of medicalreference to switch",
  })
  @ApiParam({
    name: "to",
    type: "string",
