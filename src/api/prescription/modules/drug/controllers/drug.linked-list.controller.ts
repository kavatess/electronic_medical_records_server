...
@ApiTags("drug-linked-list")
@UseGuards(AuthenticatedHeaderGuard, PoliciesGuard)
@ApiSecurity("ApiKeyAuth", ["token"])
@Controller("drug")
export class DrugLinkedListController {
  constructor(private readonly service: DrugService) {}

  @Post("linked-list/append")
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Create, Drug))
  @ApiQueries(["locale"])
  @ApiCreatedResponse({ type: DrugDto })
  @ApiErrorResponses(["409", "406", "403"])
  async append(
    @Body() item: CreateDrugDto,
    @Req() req: Request
  ): Promise<DrugDto> {
  ...
  @ApiParam({
    name: "from",
    type: "string",
    description: "_id of drug to switch",
  })
  @ApiParam({
    name: "to",
    type: "string",
