...
import { ArrayUpdateDiagnosisDto } from "../dto/array-update-diagnosis.dto";
...
export class DiagnosisController {
  ...
  @Post()
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Create, Diagnosis))
  @ApiQueries(["locale"])
  @ApiCreatedResponse({ type: Diagnosis })
  @ApiErrorResponses(["409", "406", "403"])
  async create(
    @Body() item: CreateDiagnosisDto,
    @Req() req: Request
  ): Promise<Diagnosis> {
  ...
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Create, Diagnosis))
  @ApiQueries(["locale"])
  @ApiCreatedResponse({ type: DiagnosisDto, isArray: true })
  @ApiErrorResponses(["409", "406", "403"])
  async importCreate(
    @Body() items: CreateDiagnosisDto[],
    @Req() req: Request
  ...
  @Get("count")
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Read, Diagnosis))
  ...
  @Get()
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Read, Diagnosis))
  @ApiOkResponse({ type: Diagnosis, isArray: true })
  @ApiErrorResponses(["403"])
  @ApiQueries([
    "fields",
    "locale",
  ...
  async findAll(@Query() query: Record<string, any>): Promise<Diagnosis[]> {
    const filter = query.filter ? JSON.parse(query.filter) : {};
  ...
  @KheSchema({
    ...
    filterFields: ["name", "code", "locale"],
    sortFields: ["name", "code", "locale"],
    defaultSort: ["-updatedAt"],
    view: "table",
    initialFields: ["name", "code", "locale"],
  })
  @ApiOkResponse({ type: Diagnosis, isArray: true })
  @ApiQueries([
    "fields",
  ...
  async list(@Query() query: Record<string, any>): Promise<Diagnosis[]> {
    const filter = query.filter ? JSON.parse(query.filter) : {};
    const item = await this.service.find(filter, query.fields, query);
    if (query.count) {
      item["_countDocuments"] = await this.service.countDocuments(filter);
  ...
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Manage, Diagnosis))
  @ApiErrorResponses(["403"])
  ...
  @ApiOkResponse({ type: Diagnosis, content: {} })
  ...
  @Put(":_id")
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Update, Diagnosis))
  @ApiOkResponse({ type: Diagnosis })
  @ApiErrorResponses(["404", "403"])
  @ApiQueries(["fields", "locale", "populate"])
  @ApiParam({ name: "_id", type: String, required: true })
  async updateById(
    @Param() params: { [key: string]: string },
    @Body() data: UpdateDiagnosisDto,
    @Req() req: Request
  ): Promise<Diagnosis> {
  ...
  async updateByIdAddToSet(
    @Param() params: Record<string, string>,
    @Body() data: ArrayUpdateDiagnosisDto,
    @Req() req: Request
  ): Promise<Diagnosis> {
  ...
  @Delete(":_id")
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Delete, Diagnosis))
  @ApiOkResponse({ type: Diagnosis })
  @ApiErrorResponses(["404", "403"])
  @ApiQueries(["locale"])
  @ApiParam({ name: "_id", type: String, required: true })
