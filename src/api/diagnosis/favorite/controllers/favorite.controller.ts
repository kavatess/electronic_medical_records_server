...
import { ArrayUpdateFavoriteDto } from "../dto/array-update-favorite.dto";
...
export class FavoriteController {
  ...
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Create, Favorite))
  @ApiQueries(["locale"])
  @ApiCreatedResponse({ type: FavoriteDto, isArray: true })
  @ApiErrorResponses(["409", "406", "403"])
  async importCreate(
    @Body() items: CreateFavoriteDto[],
    @Req() req: Request
  ): Promise<FavoriteDto[]> {
    const reqUser = req.headers["x-auth-user"] as string;
    const response = [];
  ...
  async findAll(@Query() query: Record<string, any>): Promise<Favorite[]> {
    const filter = query.filter ? JSON.parse(query.filter) : {};
  ...
  @KheSchema({
    dto: "FavoriteDto",
    labelField: "name",
    searchFields: ["type", "name", "value"],
    filterFields: ["user", "share", "type", "name", "value", "hit"],
    sortFields: ["user", "share", "type", "name", "value", "hit"],
  ...
  async list(@Query() query: Record<string, any>): Promise<Favorite[]> {
    const filter = query.filter ? JSON.parse(query.filter) : {};
    const item = await this.service.find(filter, query.fields, query);
    if (query.count) {
      item["_countDocuments"] = await this.service.countDocuments(filter);
  ...
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Manage, Favorite))
  @ApiErrorResponses(["403"])
  ...
  @ApiOkResponse({ type: Favorite, content: {} })
  ...
  @Put(":_id")
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Update, Favorite))
  ...
  async updateByIdAddToSet(
    @Param() params: Record<string, string>,
    @Body() data: ArrayUpdateFavoriteDto,
    @Req() req: Request
  ): Promise<Favorite> {
  ...
  @Delete(":_id")
  @CheckPolicies((ability: AppAbility) => ability.can(Action.Delete, Favorite))
  @ApiOkResponse({ type: Favorite })
  @ApiErrorResponses(["404", "403"])
  @ApiQueries(["locale"])
  @ApiParam({ name: "_id", type: String, required: true })
  async removeById(@Param() params: Record<string, string>): Promise<Favorite> {
