...
@ApiTags("diagnosissuggestion-linked-list")
@UseGuards(AuthenticatedHeaderGuard, PoliciesGuard)
@ApiSecurity("ApiKeyAuth", ["token"])
@Controller("diagnosis-suggestion")
export class DiagnosisSuggestionLinkedListController {
  constructor(private readonly service: DiagnosisSuggestionService) {}
  ...
  @ApiQueries(["locale"])
  @ApiCreatedResponse({ type: DiagnosisSuggestionDto })
  ...
  @ApiParam({
    name: "from",
    type: "string",
    description: "_id of diagnosissuggestion to switch",
  })
  @ApiParam({
    name: "to",
    type: "string",
